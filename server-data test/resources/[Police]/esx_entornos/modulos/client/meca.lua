...
AddEventHandler('playerSpawned', function(spawn)
    TriggerServerEvent('esx-meca:getJob')
end)
...
RegisterNetEvent('esx-meca:setJob')
AddEventHandler('esx-meca:setJob',function(theJob)
    job = theJob
end)

-- BLIP ENTORNO Y FORZAR

RegisterNetEvent('esx-meca:setBlip')
...
RegisterNetEvent('esx-meca:sendNotify')
...
--     if job == 'mechanic' or job == 'mechanic1' or job == 'mechanic' then 
...
RegisterCommand('meca', function(source, args)
    ...
    if args[1] == nil then
        ...
        TriggerEvent('pNotify:SendNotification', {
            text = 'Has llamado un mecanico.',
            type = "termi",
            layout = "topleft"
        })	
      --  ExecuteCommand("opp Tu llamada de entorno se ha realizado correctamente ")
        TriggerServerEvent('esx-meca:sendNotify', id, msg, streetName )
        TriggerServerEvent('esx-meca:setBlip', pos)
        TriggerEvent('esx-meca:respuesta')
    end
end)
