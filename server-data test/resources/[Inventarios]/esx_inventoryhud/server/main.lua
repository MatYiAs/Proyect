...
AddEventHandler("esx_inventoryhud:tradePlayerItem", function(from, target, type, itemName, itemCount)
		...
		TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = 'You cannot Hold this item'})
                end
            end
...
RegisterCommand("openinventory", function(source, args, rawCommand)
	if IsPlayerAceAllowed(source, "inventory.openinventory") then
		...
		if targetXPlayer ~= nil then
			...
			TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = _U("no_player") })
			TriggerClientEvent("chatMessage", source, "^1" .. _U("no_player"))
		end
		else
			TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = _U("no_permissions") })
            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'success', text = 'You purchased '..count.." "..list[i].label })
					else
						TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = 'You do not have enough money!' })
					end
				end
            end
        else
            TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = 'You do not have enough space in your inventory!' })
        end
	end
				...
				if list[i].name == item then
					...
					if not targetWeapon then
						...
						if xPlayer.getMoney() >= totalPrice then
							...
							TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'success', text = 'You purchased a '..list[i].label })
						else
							TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = 'You do not have enough money!' })
						end
					else
						TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = 'You already own this weapon!' })
					end
				end
            end
        else
		...
		for i = 1, #list, 1 do
			if list[i].name == item then
				...
				if targetWeapon then
					...
					TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = 'You do not own the weapon for this ammo type!' })
				end
            end
        end
    end
...
AddEventHandler('suku:buyLicense', function ()
	...
	if xPlayer.get('money') >= Config.LicensePrice then
		...
		TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'success', text = 'You registered a Fire Arms license.' })
