...
AddEventHandler("esx_glovebox:getItem", function(plate, type, item, count, max, owned)
    ...
    if type == "item_standard" then
      ...
      if targetItem.limit == -1 or xPlayer.canCarryItem then
            ...
            for i = 1, #coffres, 1 do
              if coffres[i].name == item then
                if (coffres[i].count >= count and count > 0) then
                  ...
                  TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = _U("invalid_quantity") } )
                end
              end
            end
        ...
        TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = _U("player_inv_no_space") } )
      end
    end
    ...
    if type == "item_account" then
      TriggerEvent("esx_glovebox:getSharedDataStore", plate, function(store)
          ...
          if (blackMoney[1].amount >= count and count > 0) then
            ...
            TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = _U("invalid_amount") } )
          end
        end)
    end
        ...
        TriggerEvent(
          ...
          function(store)
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item) * count)) > max then
              TriggerClientEvent('pNotify:client:SendAlert', source, { type = 'error', text = _U("insufficient_space") } )
            else
