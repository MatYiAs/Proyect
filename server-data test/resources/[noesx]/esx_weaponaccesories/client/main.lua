local ESX = nil
local weapons = {
    [GetHashKey('WEAPON_SNSPISTOL')] = { suppressor = GetHashKey('component_at_pi_supp_02'), flashlight = GetHashKey('COMPONENT_AT_PI_FLSH'), grip = nil, skin = GetHashKey('COMPONENT_PISTOL_VARMOD_LUXE') },
...
AddEventHandler('eden_accesories:use', function( type )
    if weapons[GetSelectedPedWeapon(PlayerPedId())] and weapons[GetSelectedPedWeapon(PlayerPedId())][type] then
        if not HasPedGotWeaponComponent(GetPlayerPed(-1), GetSelectedPedWeapon(PlayerPedId()), weapons[GetSelectedPedWeapon(PlayerPedId())][type]) then
            ...
            ESX.ShowNotification(string.format('%s %s', "Te has equipado 1 ", type))
        end
    else
        ESX.ShowNotification(string.format('%s %s %s', 'The ', type, " Ha esta arma no se le puede poner.."))
    end
end)
