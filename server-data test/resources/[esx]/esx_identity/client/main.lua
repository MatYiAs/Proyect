...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Debes crear tu identidad antes de jugar' } })
	end
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	for theData, value in pairs(myIdentity) do
		if theData == "Nombre" or theData == "Apellido" then
			reason = verifyName(value)
			
			if reason ~= "" then
				break
			end
		elseif theData == "Fecha de nacimiento" then
			if value == "Invalido" then
				reason = "Fecha de nacimiento invalida!"
				break
			end
		elseif theData == "Altura" then
			local height = tonumber(value)
			if height then
				if height > 200 or height < 140 then
					reason = "Talla invalida !"
					break
				end
			else
				reason = "Talla invalida !"
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Tu nombre es demasiado corto o demasiado largo.'
	end
	...
	if count ~= nameLength then
		return 'Tu nombre contiene caracteres especiales no válidos.'
	end
	...
	if spacesInName > 2 then
		return 'Tu nombre tiene mas de 2 espacios'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'Tu nombre debe comenzar por una letra mayuscula.'
	end
